...
export default function FaqList() {
    ...
    const fetchData = async () => { setLoading(true); try { const res = await api.get('/faqs'); if (res.data?.faqs?.length) setData(res.data.faqs); } catch { } finally { setLoading(false); } };
    useEffect(() => { fetchData(); }, []);
    if (editItem) return <FaqAdd editData={editItem} onSaved={() => { setEditItem(null); fetchData(); }} />;
    return (
        <div>
            <div className="page-header"><h1 className="page-title">List FAQ</h1><button className="btn btn-primary" onClick={() => window.location.href = '/faq/add'}><MdAdd /> Add FAQ</button></div>
